...
int viewValue_find(ViewValue_Format *vvfArray, ...)
{
    ...
    switch(vvfArray->type)
    {
        ...
        case VT_STRING_ARRAY:
            ...
            for(i = 0; i < vvfArray->vSize/sizeof(char*); i++)
            {
                if(_viewValue_compare(vvfArray->value.StringArray[i], (char*)vp))
                {
                    ret = i;
                    break;
                }
...
ViewValue_Format *viewValue_arrayAdd(ViewValue_Format *vvf, ...)
{
    ...
    switch(vvf->type)
    {
        case VT_STRING_ARRAY:
            ...
            if(tempPoi)
            {
                tempArray[len] = (char *)calloc(strlen((char*)tempPoi)+1, sizeof(char));
                strcpy((char*)tempArray[len], (char*)tempPoi);
            }
            else
    ...
    switch(vvf->type)
    {
        case VT_CHAR:
            fprintf(fd, "# %s VT_CHAR %d\n%c\\\n", 
                vvf->name, 1, vvf->value.Char);break;
        case VT_BOOL:
            fprintf(fd, "# %s VT_BOOL %d\n%c\\\n", 
                vvf->name, 1, vvf->value.Bool?'T':'F');break;
        case VT_INT:
#if(VVF_SL_SHOW_HEX)
            fprintf(fd, "# %s VT_INT %d\n%06X\\\n", 
                vvf->name, 1, vvf->value.Int);break;
#else
            fprintf(fd, "# %s VT_INT %d\n%d\\\n", 
                vvf->name, 1, vvf->value.Int);break;
#endif
        case VT_DOUBLE:
            fprintf(fd, "# %s VT_DOUBLE %d\n%.8lf\\\n", 
                vvf->name, 1, vvf->value.Double);break;
        case VT_STRING:
            fprintf(fd, "# %s VT_STRING %d\n%s\\\n", 
                vvf->name, strlen(vvf->value.String), vvf->value.String);break;
        case VT_BOOL_ARRAY:
            fprintf(fd, "# %s VT_BOOL_ARRAY %d\n", 
                vvf->name, vvf->vSize/sizeof(bool));
            for(j = 0; j < vvf->vSize/sizeof(bool); j++)
                fprintf(fd, "%c\\\n", vvf->value.BoolArray[j]?'T':'F');
            break;
        case VT_INT_ARRAY:
            fprintf(fd, "# %s VT_INT_ARRAY %d\n", 
                vvf->name, vvf->vSize/sizeof(int));
...
int viewValue_save(char *filePath, ViewValue_Format **array, int arrayLen)
{
    ...
    //
    int i;
    for(i = 0; i < arrayLen && vvf[i]; i++)
    {
        // printf("save: %p\n", vvf[i]);
        _viewValue_save(fd, vvf[i]);
    }
    //
    fclose(fd);
    //
...
int viewValue_load(char *filePath, ViewValue_Format **array, int arrayLen)
{
    if(!filePath || !array || arrayLen < 1)
        return 0;
    //
    int i = 0, hit = 0, count = 0;//匹配计数
    //指针统计成链表 方便遍历和跳过已匹配的项
    VvfLoadStat_Struct *vlss = NULL, *vlssTemp;
    if((vlss = vlssTemp = VvfLoadStat_browse(array[i++], &count)))//第一个
    {
        for(; i < arrayLen; i++)
        {
            while(vlssTemp->next)
